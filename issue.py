...
def app_version_checker(body: str) -> dict:
    ...
    if app_version:
        ...
        stable_metadata = json.loads(requests.get("https://api.github.com/repos/DGP-Studio/Snap.Hutao/releases/latest").text)
        beta_metadata = json.loads(requests.get("https://api.github.com/repos/DGP-Studio/Snap.Hutao/releases").text)[0]
        ...
        if any(app_version.startswith(version) for version in latest_version):
            ...
            return {"code": 1, "app_version": app_version, "latest_version": latest_version,
                    "data": f"请更新至最新版本: \n"
                            ...
                            f" 测试版: [{beta_metadata['tag_name']}]({beta_metadata['assets']['browser_download_url']})"}
    else:
